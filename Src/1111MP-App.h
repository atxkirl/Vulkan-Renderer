...
#define DEFAULT_FENCE_TIMEOUT 100000000000
	...
	private:
		...
		void createTextureImage()
		{
			...
			stbi_uc* pixels = stbi_load("Assets/sad cat.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
		...
		void createSwapChain()
		{
			...
			createInfo.imageArrayLayers = 1; // ???ÿ?????????????
		...
		VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
		{
			...
			else
			{
				...
				// clamp(x,low,high )??X??[low,high]???????????X?????X???low??????low?????X????high??????high??
...
//int main()
//{
//	MP::HelloTriangleApplication app;
//	try
//	{
//		app.run();
//	}
